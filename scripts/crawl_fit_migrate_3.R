...
if(B == FALSE){
  ...
  tracks <- readRDS("data/analysis/stage_delim_4/mcconnell_wB_noland5km_migrate_4.rds")
}
...
## remove individuals w/ too big gaps
tracks <- subset(tracks, !ID %in% c("224398", "224401") )
# tracks <- subset(tracks, !ID %in% c("224398", "224389", "224401") )
...
## w/out B locations
if(B == FALSE){
  saveRDS(fit.kf, "data/analysis/crawl_3/fit_list_3_noB_migrate.rds")
  saveRDS(plocs_list, "data/analysis/crawl_3/prediction_single_list_3_noB_migrate.rds")
} else {
  saveRDS(fit.kf, "data/analysis/crawl_3/fit_list_3_wB_migrate.rds")
  saveRDS(plocs_list, "data/analysis/crawl_3/prediction_single_list_3_wB_migrate.rds")
}


## Visualize one individual's prediction vs. observed locations 
one_p <- plocs_list[[10]] # one individual
one_o <- olocs_list[[10]]
...
mapview::mapview(pred_line) + mapview::mapview(obs_sf, col.regions="red")
...
for(i in seq_along(plocs_list)){
  ...
  ggplot() +
    ...
    coord_sf(
      xlim = c(xtnt[1]-.05, xtnt[3]+.05), ylim = c(xtnt[2], xtnt[4]), expand = T) +
    theme_bw() +
  ...
  if(B == FALSE){
    ...
    ggsave(paste0("figures/crawl_pred/maps/", id, "wB.png"), width = 7, height=6)
  }
...
if(B == FALSE){
  saveRDS(plocs, "data/analysis/crawl_3/prediction_single_noB_noland5km_migrate_3.rds")
} else {
  saveRDS(plocs, "data/analysis/crawl_3/prediction_single_wB_noland5km_migrate_3.rds")
  }
