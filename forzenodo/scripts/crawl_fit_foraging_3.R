## Fit crawl model to filtered locations and make predictions -- # FORAGING #
...
if(B == FALSE){
  tracks <- readRDS("data/analysis/stage_delim_4/mcconnell_noB_noland500m_forage_4.rds")
} else {
  tracks <- readRDS("data/analysis/stage_delim_4/mcconnell_wB_noland500m_forage_4.rds")
}
...
for(i in seq_len(n_ids)){
  ...
  ## remove some bad points at end for one individual
  
  if(id == "213041"){
    one <- subset(one, sdafilter != "end_location")
  }
...
## w/out B locations
if(B == FALSE){
  saveRDS(fit.kf, "data/analysis/crawl_3/fit_list_3_noB_noland500m_forage.rds")
  saveRDS(plocs_list, "data/analysis/crawl_3/prediction_single_list_3_noB_noland500m_forage.rds")
} else {
  # saveRDS(fit.kf, "data/analysis/crawl_3/fit_list_3_wB_forage.rds")
  # saveRDS(plocs_list, "data/analysis/crawl_3/prediction_single_list_3_wB_forage.rds")
  saveRDS(fit.kf, "data/analysis/crawl_3/fit_list_3_wB_noland500m_forage.rds")
  saveRDS(plocs_list, "data/analysis/crawl_3/prediction_single_list_3_wB_noland500m_forage.rds")
}
...
obs_line <- obs_sf %>% summarise(do_union=FALSE) %>% st_cast("LINESTRING")
...
mapview::mapview(obs_line) + mapview::mapview(obs_sf)
...
land_smooth <- smoothr::smooth(land)
...
for(i in seq_along(plocs_list)){
  ...
  xtnt <- st_bbox(pred_sf)
  
  if(sex == "F") {cl <- "red"} else {cl <- "blue"}
  
  ## map it
  ggplot() +
    # borders("world", colour = NA, fill="grey65") +
    geom_sf(data=land_smooth, fill="grey70") +
    geom_sf(
      data = obs_sf, color="black", alpha=1, size=0.25, inherit.aes = FALSE) +
    geom_sf(
      data = pred_line, color=cl, alpha=1, size=0.25, inherit.aes = FALSE) +
    # geom_sf(data = land, inherit.aes = FALSE, fill="grey65", color=NA) +
    coord_sf(
...
ggplot() + 
  ...
  geom_path(data=plocs, aes(x = DateTime, y = sqrt(nsd)/1000, color = sex),
            size=0.75) + 
  xlab("") + ylab("Net displacement (km)") + 
  theme_bw() + 
  facet_wrap(~ID)

ggsave("figures/nsd/nsd_crawlpreddata_wB_forage_n19.png", width=6,height=5)

### Save single prediction data ------------------------------------------------

if(B == FALSE){
  saveRDS(plocs, "data/analysis/crawl_3/prediction_single_noB_nsd_noland500m_forage_3.rds")
} else {
  saveRDS(plocs, "data/analysis/crawl_3/prediction_single_wB_nsd_noland500m_forage_3.rds")
}
